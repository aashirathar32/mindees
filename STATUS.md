...
model bridge and the iOS Simulator bridge test routes a `UIButton` `.touchUpInside`
target/action callback through `MindeesApp.dispatchEvent(handlerId)` and observes
the JS-driven label update. This is still not a full Phase 8F completion:
...
presses a native `UIButton` and observes the JS-driven label update. **What is NOT
...
| iOS embedded-runtime bridge (JavaScriptCore + JS↔native command bridge) | 🧪 in progress (Phase 8F-C) - `examples/native-hosts/ios`; model bridge tested via `swift test` + UIKit `UIButton` smoke tested on iOS Simulator in CI; physical-device smoke pending |
...
- **Native iOS/Android renderer backends.** The platform-neutral **native command
  ...
  `UIButton` smoke test (Phase 8F-C). What remains research-track/planned is
