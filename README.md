...
> The **signed OTA core** (Pulse — manifest, Ed25519 signing, content-addressed
> store, atomic rollback) landed in Phase 9A. Still missing: iOS embedded-runtime
> parity and physical-device proof, so production native mobile apps are not ready.
>
...
SSR), compiler, CLI, typed router, and native rendering foundation all work now.
The repo also has CI-verified iOS/Android host projects and an Android example
app that runs on an emulator through an embedded QuickJS JS↔native bridge. Still
